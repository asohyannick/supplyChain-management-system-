...
const showBlockChainDeliveryLog = async(req: Request, res: Response): Promise<Response> => {
    try {
        const { id } = req.params;
        const blockDeliveryLog = await BlockChainDelivery.findById(id);
        if (!blockDeliveryLog) {
...
export default showBlockChainDeliveryLog;